...
async def codeforces_submissions(submissions: Submissions, user_id: int = Depends(get_current_user),
    ...
    if not user.codeforces_api_key:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Codeforces API is not configured')
    ...
    submissions_result = await format_codeforces_submissions(response, user_id, contest.id, db)
