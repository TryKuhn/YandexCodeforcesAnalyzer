...
def format_codeforces_standings(
    ...
    for row in rows:
        party = row["party"]
        login = (
            party["members"][0]["handle"]
            if "teamId" not in party
            else f'team_{party["teamId"]}'
        )
        ...
        for idx, problem in enumerate(row["problemResults"]):
            ...
            if points > 0 and start_time and "bestSubmissionTimeSeconds" in problem:
                success_time = start_time + timedelta(
                    seconds=problem["bestSubmissionTimeSeconds"]
                )
...
async def format_codeforces_submissions(
    ...
    for submission in submissions:
        ...
        problem = await db.execute(
            select(Task).filter_by(full_name=problem["name"], contest_id=contest_id)
        )
        ...
        participant = await db.execute(
            select(ContestParticipant).filter_by(
                contest_id=contest_id, login=participant_login
            )
        )
