...
async def yandex_standings(standings: YandexStandings, user_id: int = Depends(get_current_user),
    ...
    async with httpx.AsyncClient() as client:
        contest_info = await client.get(f'{settings.YANDEX_HOST}/{method_name}', headers=headers)
        ...
        standings_data = await client.get(f'{settings.YANDEX_HOST}/{method_name}/standings', params=params,
                                          headers=headers)
    ...
    contest, tasks, rows = format_yandex_standings(contest_info, standings_data, user_id, standings.show_unofficial)
