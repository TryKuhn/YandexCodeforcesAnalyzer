...
async def yandex_submissions(submissions: YandexSubmissions, user_id: int = Depends(get_current_user),
    ...
    if not user.yandex_access_token:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Yandex API is not configured')
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        submissions_info = await client.get(f'{settings.YANDEX_HOST}/{method_name}', headers=headers, params=params)

        if submissions_info.status_code == 400:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Contest standings are not available')
        elif submissions_info.status_code == 401:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Your token expired or invalid')
        elif submissions_info.status_code == 403:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail='You do not have permission to contest')
        elif submissions_info.status_code == 404:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Contest not found')
        elif submissions_info.status_code != 200:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Failed to fetch standings')

        submissions_info = submissions_info.json()['submissions']

        async def fetch_source(submission_info):
            source_url = f'{settings.YANDEX_HOST}/{method_name}/{submission_info["id"]}/source'
            try:
                submission_source = await client.get(source_url, headers=headers)
                submission_info['source'] = submission_source.text if submission_source.status_code == 200 else ""
            except Exception:
    ...
    contest = await db.execute(select(Contest).filter_by(external_id=submissions.contest_id))
    contest = contest.scalars().first()

    submissions_result = await format_yandex_submissions(submissions_info, user_id, contest.id, db)
