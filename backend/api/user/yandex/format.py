...
from models import ContestParticipant, Contest, Task, TaskResult, Submission


def format_yandex_standings(contest_info: dict, standings: Dict[str, dict], user_id: int,
                            unofficial: bool) -> Tuple[Contest, List[Task], List[ContestParticipant]]:
    ...
    if 'startTime' in contest:
        try:
            start_time = datetime.fromisoformat(contest['startTime'].replace('Z', '+00:00')).replace(tzinfo=None)
        except:
            start_time = None
...
async def format_yandex_submissions(submissions: Tuple[Any], user_id: int, contest_id: int, db: AsyncSession) -> \
List[Submission]:
    formatted_submissions = []
    for submission in submissions:
        print(submission)

        problem = await db.execute(select(Task).filter_by(short_name=submission['problemAlias'], contest_id=contest_id))
        problem = problem.scalars().first()

        participant_name = submission['author']

        participant = await db.execute(
            select(ContestParticipant).filter_by(contest_id=contest_id, name=participant_name))
        ...
        formatted_submission = Submission(
            ...
            send_time=datetime.fromisoformat(submission['submissionTime'].replace('Z', '+00:00')).replace(tzinfo=None),
