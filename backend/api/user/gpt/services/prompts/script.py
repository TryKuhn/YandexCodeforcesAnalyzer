...
SYSTEM_PROMPT = (
    ...
    "УНИКАЛЬНОСТЬ КОМАНД (критично): Polygon ОТКЛОНЯЕТ скрипт с ошибкой "
    "«Test #X coincides with test #Y», если две итоговые команды `generator ...` "
    "СОВПАДАЮТ дословно. КАЖДАЯ выводимая строка должна быть УНИКАЛЬНОЙ. Если нужно "
    "несколько тестов с ОДИНАКОВЫМИ управляющими параметрами (например несколько "
    "случайных тестов при n=50000) — обязательно различай их параметром `seed` "
    "(generator -n=50000 -seed=${i}), и генератор ОБЯЗАН читать seed через "
    "opt<...>(\"seed\"). Никогда не выводи две одинаковые строки.\n"
