...
async def _polygon_call(method_name: str, params: dict, user: User):
    params = {
        "apiKey": user.polygon_api_key,
        "time": str(int(time())),
        **params,
    }
    signature = create_signature(method_name, params, str(user.polygon_api_secret))
...
async def get_problem_solutions(
    ...
    return (
        await _polygon_call("problem.solutions", {"problemId": str(problem_id)}, user)
        or []
    )


async def view_solution(
    problem_id: int, name: str, user_id: int, db: AsyncSession
) -> str:
    """Returns raw source code of a solution."""
    user = await _get_user(user_id, db)
    result = await _polygon_call(
        "problem.viewSolution", {"problemId": str(problem_id), "name": name}, user
    )
...
async def get_problem_files(problem_id: int, user_id: int, db: AsyncSession) -> dict:
    ...
    user = await _get_user(user_id, db)
    return (
        await _polygon_call("problem.files", {"problemId": str(problem_id)}, user) or {}
    )


async def view_file(
    problem_id: int, file_type: str, name: str, user_id: int, db: AsyncSession
) -> str:
...
async def get_problem_script(
    problem_id: int, testset: str, user_id: int, db: AsyncSession
) -> str:
    """Returns the generator script for a testset."""
    user = await _get_user(user_id, db)
    result = await _polygon_call(
        "problem.script", {"problemId": str(problem_id), "testset": testset}, user
    )
    return result.get("message", "") if isinstance(result, dict) else str(result)


async def get_problem_tests(
    problem_id: int, testset: str, user_id: int, db: AsyncSession
) -> list[dict]:
...
async def get_problem_tags(
    ...
    user = await _get_user(user_id, db)
    result = await _polygon_call(
        "problem.viewTags", {"problemId": str(problem_id)}, user
    )
