...
from settings import DEFAULT_PAGE_SIZE, YANDEX_HOST


async def submission(
    client: ClientSession, token: str, contest_id: str, submission_id: str
) -> str:
...
async def submissions(
    ...
    async with ClientSession(connector=connector) as client:
        submission_list = await submissions_info(
            client, token, contest_id, from_pos, to_pos
        )

        tasks = [
            create_task(
                submission(client, token, contest_id, str(submission_info["id"]))
            )
