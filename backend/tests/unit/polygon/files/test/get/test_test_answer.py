import api.user.polygon.files.test.get.test_answer as mod
...
async def test_get_test_answer_params(monkeypatch, db, user):
    cap = _patch(monkeypatch, user, ret={"message": "HELLO"})
    await get_test_answer(5, "tests", 3, user.id, db)
    assert cap["method"] == "problem.testAnswer"
    assert cap["params"] == {"problemId": "5", "testset": "tests", "testIndex": "3"}


async def test_get_test_answer_message_extraction(monkeypatch, db, user):
    _patch(monkeypatch, user, ret={"message": "HELLO"})
    result = await get_test_answer(5, "tests", 3, user.id, db)
    assert result == "HELLO"


async def test_get_test_answer_non_dict(monkeypatch, db, user):
