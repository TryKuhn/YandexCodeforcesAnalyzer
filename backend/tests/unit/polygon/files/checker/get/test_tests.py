import api.user.polygon.files.checker.get.tests as mod
from api.user.polygon.files.checker.get.tests import get_checker_tests
...
async def test_get_checker_tests_calls_method(monkeypatch, db, user):
    cap = _patch(monkeypatch, user)
    await get_checker_tests(5, user.id, db)
    assert cap["method"] == "problem.checkerTests"
