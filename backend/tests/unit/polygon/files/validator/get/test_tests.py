import api.user.polygon.files.validator.get.tests as mod
from api.user.polygon.files.validator.get.tests import get_validator_tests
...
async def test_get_validator_tests_calls_method(monkeypatch, db, user):
    cap = _patch(monkeypatch, user)
    await get_validator_tests(5, user.id, db)
    assert cap["method"] == "problem.validatorTests"
