import api.user.polygon.files.validator.get.validator as mod
...
async def test_get_validator_calls_method(monkeypatch, db, user):
    cap = _patch(monkeypatch, user)
    await get_validator(5, user.id, db)
    assert cap["method"] == "problem.validator"
