import api.user.polygon.files.interactor.get.interactor as mod
from api.user.polygon.files.interactor.get.interactor import get_interactor
...
async def test_get_interactor(monkeypatch, db, user):
    cap = _patch(monkeypatch, user)
    await get_interactor(5, user.id, db)
    assert cap["method"] == "problem.interactor"
