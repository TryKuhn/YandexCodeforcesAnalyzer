...
class PairOfBannedSubmissions(Base):
    ...
    contest: Mapped["Contest"] = relationship(
        back_populates="pairs_of_banned_submissions"
    )
    report: Mapped["PlagiarismReport"] = relationship(back_populates="pairs")

    first_submission: Mapped["Submission"] = relationship(
        foreign_keys=[first_submission_id], back_populates="banned_as_first"
    )
    second_submission: Mapped["Submission"] = relationship(
        foreign_keys=[second_submission_id], back_populates="banned_as_second"
    )
